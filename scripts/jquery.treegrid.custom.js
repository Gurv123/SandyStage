...
(function($) {
    ...
    function _initTree(options) {
        //settings
        var $this = $(this),
            settings = $.extend({}, $.fn.treegrid.defaults, options);
    ...
    function _add($this, items) {
        var $target = _getBranch($this).last().next(),
            childs = [],
            parentId = _getId($this);
        //cell count
        var $table = $this,
            cellCount;
        ...
        $.each(items, function(i, item) {
            ...
            /*if (cnt < cellCount) {
                for (var i = $tr.find('>td').length; i < cellCount; i++) $tr.append('<td>');
                console.log('ici');
            } else if (cnt > cellCount) {
    ...
    function _getId($this) {
        ...
        //var template = /(?:treegrid([_][A-Fa-f0-9]+)+)|(?:treegrid([-][A-Fa-f0-9]+)+)/;
        //var template = /treegrid([-_][A-Fa-f0-9]+)+/;
    ...
    function _getParentId($this) {
        ...
        //var template = /(?:treegrid-parent([_][A-Fa-f0-9]+)+)|(?:treegrid-parent([-][A-Fa-f0-9]+)+)/;
        //var template = /treegrid-parent([-_][A-Fa-f0-9]+)+/;
        if (template.test($this.attr('class'))) {
            return template.exec($this.attr('class'))[1];
        };
        return null;
    ...
    var $moveItem = null,
        ...
        $moveOver = null,
        $indicator,
        downX, downY, offX, offY, isDragging = false,
        isTarget = false,
        dropMap, position, expandTimer = false;
    //move events
    function _nodeMouseDown(e) {
        ...
        //move enabled?
        var $this = $(this),
            settings = $this.closest('table').data('treegrid-settings');
    ...
    function _dragStart(e) {
        ...
        //event callback
        var $treegrid = $moveItem.closest('table'),
            settings = $treegrid.data('treegrid-settings');
